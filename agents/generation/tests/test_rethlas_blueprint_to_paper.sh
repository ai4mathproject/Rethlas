...
require_text 'deliverable inventory' "$skill_file"
require_text "from the user's request and the" "$skill_file"
require_text 'Check each item against `blueprint_verified\.md`' "$skill_file"
require_text 'missing item is a scope mismatch' "$skill_file"
require_text 'Stop and report the exact missing items' "$skill_file"
require_text 'The caller may then submit an expanded problem' "$skill_file"
require_text 'resume this skill only after a new verified blueprint exists' "$skill_file"
